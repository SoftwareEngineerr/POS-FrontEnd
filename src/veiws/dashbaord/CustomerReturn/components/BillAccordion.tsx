...
// import Items from "../../../../layouts/full/customizemenu/components/items";
...
const BillAccordion = ({ bill }) => {
  ...
  const [returnproducts , setReturnproducts ] = useState(0)
  ...
  const fetchItems = async () => {
    ...
    const res = await dispatch(
      GetRequest(`${url.ShowCustomerBillId}${bill.id}`, Token)
    );
...
const Addproduct = (getvalue , e) => {
    // console.log(getvalue , e)
    // console.log(selectedItem.length)
    const findData = selectedItem.filter((getitem)=>getitem.id != getvalue.id)
    // console.log(findData)
    if(e.target.checked){
        const data = [
            ...selectedItem,
            {...getvalue , returnquantity: 0}
        ]
    ...
    >
      ...
      <AccordionSummary
        ...
        <ReturnProductPopup
            ...
            billid={bill.id}
            resetselecteditems={()=>setSelectedItem([])}
            onSubmit={(data) => {
                returnItemAPI({
                ...data,
        ...
        <TableContainer
            ...
            <Table>
                ...
                <TableRow sx={{ background: "#f1f5f9" }}>
                    ...
                    <TableCell sx={{ fontWeight: 700 }}>Return Quantity</TableCell>
                ...
                {products?.items?.map((p, i) => (
                    <Item Addproduct={Addproduct} selectedItem={selectedItem} setSelectedItem={(e)=>updateproducts(e)}  p={p} i={i} />
...

            {
                ...
                <Box
                    ...
                    <Components.CustomBtn data="return"
                        click={() => {
                            // setSelectedProduct();
                            setOpenReturn(true);
                            }}
