...
const ListAccordionDetails = ({ supplier }) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const res = await dispatch(
        GetRequest(`${State.Api.ShowCustomerBill}/${supplier}`, Token)
      );
      ...
      .map((item) => {
        if (item.type === "BILL") {
          return {
            ...
            debit: 0,
            credit: Number(item.total_amount || 0),
  ...
  return (
    <Box>
      ...
      {/* SUMMARY */}
      <Box sx={{ mb: 2 }}>
        <Typography>Total Purchase: {totalCredit}</Typography>
        <Typography>Total Paid: {totalDebit}</Typography>
